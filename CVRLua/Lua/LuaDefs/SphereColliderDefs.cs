...
namespace CVRLua.Lua.LuaDefs
{
    static class SphereColliderDefs
    {
        ...
        {
            p_vm.RegisterClass(typeof(SphereCollider), null, ms_metaMethods, StaticGet, null, InstanceGet, InstanceSet);
        }
        ...
        {
            p_reader.PushObject(new Wrappers.Vector3((p_obj as SphereCollider).center));
        }
        static void SetCenter(object p_obj, LuaArgReader p_reader)
        {
            ...
            {
                (p_obj as SphereCollider).center = l_vec.m_vec;
        ...
        {
            p_reader.PushNumber((p_obj as SphereCollider).radius);
        }
        static void SetRadius(object p_obj, LuaArgReader p_reader)
        {
            ...
            if(!p_reader.HasErrors())
            {
                (p_obj as SphereCollider).radius = l_radius;
                p_reader.PushBoolean(true);
            }
            else
                p_reader.PushBoolean(false);
